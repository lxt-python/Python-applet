...
def stamap():
    ...
    width=0.5
    x_bar=np.arange(14)
    rect=ax.bar(left = 0, bottom = x_bar, width = sizes2, height = width, color = colorss, orientation = "horizontal" )
    for rec in rect:
        x=rec.get_y()
        height=rec.get_width()
        ax.text(1.02*height,x+0.1,str(height))
 
 
    ax.set_yticks(x_bar)
    ax.set_yticklabels(labels2)
    ax.set_xlabel("aver_unit_price")
    ax.set_title("武汉房源均价条形图")
    ax.grid(True)
    plt.savefig('./whfyjj1.png')
    plt.show()
